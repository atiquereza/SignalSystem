...
namespace SignalSystem.Libs
{
    public class DBGateway
    {
        ...
        public DBGateway()
        {
            try
            {
                conString = ConfigurationManager.ConnectionStrings["SignalSystemConnectionString"].ConnectionString;
                SqlConnectionObj = new MySqlConnection(conString);
                SqlCommandobj = SqlConnectionObj.CreateCommand();
            }
            catch (Exception exception)
            {
                conString = "Server=localhost;Port=3306;Database=signalappdb;Uid=root;password='';Convert Zero Datetime = true";
                SqlConnectionObj = new MySqlConnection(conString);
                SqlCommandobj = SqlConnectionObj.CreateCommand();
            }
        }
